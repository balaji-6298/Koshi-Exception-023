...
import {  Historicalhomes, categories } from '../Bottom Navbar constants/data';
...
export const Historicalhomess = () => {
  const [obj, setObj] = useState<categories[]>([]);
  const [arr, setArr] = useState<Historicalhomes[]>([]);
  ...
  useEffect(() => {
    if (obj.length > 0) {
      const pools: Historicalhomes[] = obj.flatMap((ele) => ele.Historicalhomes);
      setArr(pools);
    }
  }, [obj]);
